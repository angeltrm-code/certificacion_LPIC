...
adduser usuario-nominal
passwd usuario-nominal

/etc/sudoers se edita con visudo y añadimos al final del archivo :

visudo

usuario-nominal ALL = (ALL)  /bin/vi /etc/httpd/conf/httpd.conf, /usr/bin/systemctl restart httpd, /usr/bin/less,  /usr/bin/passwd [A-z]*, !/usr/bin/passwd root

#usuario-nominal ALL = (ALL)  NOEXEC: /bin/vi /etc/httpd/conf/httpd.conf, /usr/bin/systemctl restart httpd,/usr/bin/less, /bin/rm

##Entramos en el sistema con el usuario-nominal siempre el password es el del usuario nominal:
sudo -l
