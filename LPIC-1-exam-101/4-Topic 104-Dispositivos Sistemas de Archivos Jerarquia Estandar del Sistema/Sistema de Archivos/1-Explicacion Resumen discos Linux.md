...
Número máximo de particiones  4 particiones primarias (o 3 primarias + 1 extendida)	Hasta 128 particiones en Windows (más en otros SOs)
Tamaño máximo de disco	      2 TB	                                                 9.4 ZB (Zettabytes)
Método de almacenamiento de particiones	Tabla de particiones almacenada en el sector de arranque (512 bytes)	Múltiples copias de la tabla de particiones a lo largo del disco
Seguridad y redundancia	No tiene redundancia	                       Almacena copias de seguridad de la tabla de particiones
...
Se almacena en el primer sector del disco y contiene información sobre la tabla de particiones y el código de arranque.
Limitaciones:
Solo soporta 4 particiones primarias. Si se requieren más, se debe usar una partición extendida con particiones lógicas dentro.
No permite gestionar discos mayores a 2 TB.
No tiene mecanismos de redundancia, por lo que si el MBR se corrompe, el disco puede volverse inservible.
...
Permite gestionar discos de más de 2 TB.
Almacena copias de seguridad de la tabla de particiones en diferentes ubicaciones del disco.
Usa sumas de verificación CRC32 para detectar y corregir errores en la tabla de particiones.
...
Necesitas compatibilidad con Windows XP o versiones antiguas de Linux.

✅ Usar GPT si:
Usas UEFI en lugar de BIOS.
El disco tiene más de 2 TB.
Necesitas más de 4 particiones primarias.
...
mklabel gpt	                    Crea una nueva tabla de particiones GPT en el disco. (Borra todos los datos)
mklabel msdos	                Crea una tabla de particiones MBR (para BIOS legacy).
...
Conectas LUNs nuevas
*Los tres valores representan canal, ID de destino SCSI y LUN.
*Los guiones actúan como comodines que significan "volver a escanear todo".
...
Útil en entornos VMware, KVM, SAN, iSCSI y Fibre Channel.
...
Cantidad total y disponible de inodos y bloques.
Estado del sistema de archivos (montado, desmontado, dañado).
Fecha y hora del último montaje y modificación.
Ubicación del primer inodo y la estructura de bloques.
...
suid → Permite ejecutar archivos con setuid/setgid.
dev → Permite acceso a dispositivos.
exec → Permite ejecución de binarios.
auto → Se monta automáticamente en el arranque.
...
Ejemplo: Para montar /mnt/datos, la unidad debe llamarse mnt-datos.mount.
systemd usa mount y umount de forma automática cuando se inicia el sistema o se detiene el servicio.
...
✅ Desfragmentación en línea: Puede desfragmentarse sin desmontar.
✅ Compatibilidad con EXT2 y EXT3: Puede montar y actualizar sistemas anteriores.
...
-f a fsck para forzar la comprobación
...
✅ Snapshots y cuota de disco: Soporta cuotas de usuarios y snapshots con xfsdump y xfsrestore.
✅ No se puede reducir (shrink): A diferencia de ext4, no permite reducir su tamaño, solo expandir.
...
Copia y backup: xfs_copy, xfs_metadump, xfs_mdrestore
...
Compresión transparente para reducir el uso de espacio.
...
¿Se puede usar Btrfs en producción con Red Hat Enterprise Linux (RHEL)?
Btrfs no está soportado oficialmente en Red Hat Enterprise Linux (RHEL) desde RHEL 8 en adelante. Red Hat eliminó el soporte de Btrfs en RHEL 7.4 y versiones posteriores, y no lo incluyó en RHEL 8 y RHEL 9.
