# Clase 6 Martes 03 03 2026

Apuntes de clase sobre Clase 6 Martes 03 03 2026, reorganizados para facilitar la consulta rápida y el repaso.
...
## Finalizacion Tema 104: Dispositivos, sistemas de archivos Linux y el estándar de jerarquía de archivos Material Alumnos LPIC-1-2025\LPIC-1-exam-101\4-Topic 104-Dispositivos Sistemas de Archivos Jerarquia Estandar del Sistema\Sistema de Archivos -->Explicacion Resumen discos Linux.txt -->Explicacion Gestion de Swap.txt

## Finalizacion Tema 103: Comandos GNU y Unix
...
## Añadimos la linea al final del archivo

```bash
vi  /root/.bash_profile
export PS1='\[\e[1;36m\]\u\[\e[0m\]@\[\e[1;33m\]\h \[\e[1;37m\]\w\[\e[0m\] \[\e[1;32m\]\$\[\e[0m\] '
```

## Para aplicar los cambios podemos iniciar session o ejecurar sin salir de la session:

```bash
source /root/.bash_profile
```

## Explicación por partes de la variable PS1

\[\e[1;36m\] → cambia el color del texto a cian brillante.

\u → nombre del usuario actual.

\[\e[0m\] → restablece el color a normal.

@ → carácter literal @.

\[\e[1;33m\] → cambia el color a amarillo brillante.

\h → nombre del host (máquina).

\[\e[0m\] → restablece color.

Espacio.

\[\e[1;37m\] → cambia el color a blanco brillante.
...
\[\e[1;32m\] → cambia el color a verde brillante.

\$ → símbolo del prompt ($ para usuario normal, # si eres root).
...
## Laboratorio creacion de usuarios y grupos:
...
Crear el usario1 grupo primario informatica y no tiene shell adduser -g informatica -s /sbin/nologin usuario1

```bash
passwd usuario1
```

Crear el usuario2 y la cuenta le caduca mañana  01/10/2025 adduser -e 2025-10-01 usuario2
...
Crear el usuario3 y que en el primer inicio de session le pida el cambio de password adduser usuario3
...
## Cuando entre con el usuario3 el password que colocare sera por ejemplo Password,319

### El usuario4 tiene que caducar la cuenta el 03/10/2025

### adduser -e 2025-10-03 usuario4
...
Crear el usuario5 tiene que tener el alias de comando listar y sea un ls com permisos largos y ordene los ficheros por fecha de modificacion Tiene instalado java en el directorio /usr/local/jdk1.7 tiene que esta la varible JAVA_HOME
...
```bash
vi  /home/usuario5/.bash_profile
JAVA_HOME=/usr/local/jdk1.7
export JAVA_HOME
```

## Comprobar el correcto funcionamiento del usuario5:

### [usuario5@rockylinux8 ~]$ echo $JAVA_HOME

[usuario5@rockylinux8 ~]$ alias [usuario5@rockylinux8 ~]$ listar
...
Ver la ultima vez que se han conectado todos los usuarios Ver los inicios de session de los ultimos 15 dias. Ver los usuarios conectados actualmente al sistema. Bloquear al ususario becario
